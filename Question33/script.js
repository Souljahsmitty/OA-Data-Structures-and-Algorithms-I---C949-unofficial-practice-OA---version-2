    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! The `hash(object)` function speeds up dictionary key comparisons by computing a unique hash value for each key, enabling efficient lookups.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is **B. The `hash(object)` function speeds up dictionary key comparisons by computing a unique hash value for each key, enabling efficient lookups.";
				feedback.className = "wrong";
			}
        });
    } else {
