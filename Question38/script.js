    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "A") {
				feedback.textContent = "✔ Correct! The `Contains(Object x)` method checks if an element is present in a list and returns true if found, otherwise false.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is A. The `Contains(Object x)` method checks if an element is present in a list and returns true if found, otherwise false.";
				feedback.className = "wrong";
			}
        });
    } else {
